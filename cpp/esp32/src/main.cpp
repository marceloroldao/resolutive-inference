...
const int16_t observations[kLength][7] = {
...
const uint8_t expected[kLength] = {
...
bool parity_test() {
    uint8_t output[kLength]{};
    if (!kernel.decode(model, &observations[0][0], kLength, output)) {
        return false;
    }
...
void benchmark() {
    ...
    for (uint32_t i = 0; i < 32; ++i) {
        kernel.decode(model, &observations[0][0], kLength, output);
    }
    ...
    const float us_per_sequence = static_cast<float>(elapsed) / kIterations;
    const float us_per_observation = us_per_sequence / kLength;

    Serial.printf("sizeof(model): %u bytes\n", static_cast<unsigned>(sizeof(model)));
    Serial.printf("sizeof(kernel): %u bytes\n", static_cast<unsigned>(sizeof(kernel)));
    Serial.printf("core sizeof total: %u bytes\n",
                  static_cast<unsigned>(sizeof(model) + sizeof(kernel)));
    Serial.printf("algorithmic core accounting: %u bytes\n",
                  static_cast<unsigned>(IntegerLag8Kernel::core_bytes()));
    Serial.printf("latency: %.3f us/sequence\n", us_per_sequence);
    Serial.printf("latency: %.3f us/observation\n", us_per_observation);
}
...
void setup() {
    ...
    Serial.println("Resolutive Inference ESP32 integer lag-8 harness");
    const bool parity_ok = parity_test();

...
@dataclass
class GaussianHMM:
    """Known-parameter HMM with diagonal Gaussian emissions."""
    ...
    def parameter_count(self) -> int:
        """Count model parameters, excluding the transient posterior."""
        return int(self.transition.size + self.means.size + self.variances.size)


@dataclass
class StudentTHMM(GaussianHMM):
    """Known-parameter HMM with independent Student-t emissions."""
